...
export default function Home() {
  ...
  const myEmail = "wangcathy633@gmail.com"; // 已更新 Email
  ...
  const socialLinks = {
    ...
    tiktok: "https://www.tiktok.com/@wangcathy3", // 已填入 TikTok 链接
    linkedin: "#" // 这里预留了 LinkedIn 占位符，有链接后可随时填入
  };
  ...
  // SVG 图标代码 (已增加 TikTok 和 LinkedIn 占位符)
  const icons = {
    whatsapp: (
    ...
    instagram: (
      <svg width="32" height="32" viewBox="0 0 24 24" fill="#E4405F"><path d="M7.8 2h8.4c3.2 0 5.8 2.6 5.8 5.8v8.4c0 3.2-2.6 5.8-5.8 5.8H7.8C4.6 22 2 19.4 2 16.2V7.8A5.8 5.8 0 017.8 2m-.2 2A3.6 3.6 0 004 7.6v8.8c0 2 1.6 3.6 3.6 3.6h8.8c2 0 3.6-1.6 3.6-3.6V7.6A3.6 3.6 0 0016.4 4H7.6M12 7a5 5 0 015 5 5 5 0 01-5 5 5 5 0 01-5-5 5 5 0 015-5m0 2a3 3 0 00-3 3 3 3 0 003 3 3 3 0 003-3 3 3 0 00-3-3m5.1-.9a1.1 1.1 0 11-1.1 1.1 1.1 1.1 0 011.1-1.1z"></path></svg>
    ),
    tiktok: (
      <svg width="32" height="32" viewBox="0 0 448 512" fill="#25F4EE"><path d="M448 209.91a210.06 210.06 0 0 1-122.77-39.25V349.38A162.55 162.55 0 1 1 185 188.31V278.2a74.62 74.62 0 1 0 52.23 71.18V0h88a121.18 121.18 0 0 0 1.86 22.17A122.18 122.18 0 0 0 381 102.39a121.4 121.4 0 0 0 67 20.14z"></path></svg>
    ),
    linkedin: (
      <svg width="32" height="32" viewBox="0 0 24 24" fill="#0A66C2"><path d="M19 3a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h14m-.5 15.5v-5.3a3.26 3.26 0 00-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 011.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 001.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 00-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"></path></svg>
    )
  };
  ...
  return (
    <div style={{ backgroundColor: '#f0fdf4', minHeight: '100vh', fontFamily: 'sans-serif', color: '#1e293b' }}>
      ...
      <header style={heroStyle}>
        ...
        <p style={{ fontSize: '20px', opacity: 0.9, maxWidth: '750px', margin: '0 auto', lineHeight: '1.6' }}>
      ...
      <footer style={{ backgroundColor: '#fff', padding: '80px 20px', textAlign: 'center', borderTop: '1px solid #e2e8f0' }}>
        ...
        {/* 彩色图标区 (增加 TikTok 和 LinkedIn) */}
        <div style={{...iconGroupStyle, marginBottom: '40px'}}>
          <a href={socialLinks.facebook} target="_blank" rel="noopener noreferrer" title="Facebook">{icons.facebook}</a>
          <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" title="Instagram">{icons.instagram}</a>
          <a href={socialLinks.tiktok} target="_blank" rel="noopener noreferrer" title="TikTok">{icons.tiktok}</a>
          <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" title="LinkedIn">{icons.linkedin}</a>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
          <a href={socialLinks.whatsapp} target="_blank" rel="noopener noreferrer" style={{ ...btnStyle, padding: '15px 40px', fontSize: '18px' }}>
            {icons.whatsapp} WhatsApp Us: +86 188 3109 7125
          </a>
          {/* 这里是更新后的 Email 地址 */}
          <p style={{ color: '#64748b', marginTop: '20px' }}>Email: {myEmail}</p>
          <p style={{ fontSize: '14px', color: '#94a3b8', marginTop: '30px' }}>
